...
export function useCreateSet() {
  ...
  const createSet = async (
    ...
    try {
      console.log('[useCreateSet] Creating set for:', { exerciseTitle, weight, repetition });
      ...
      console.log('[useCreateSet] Exercise search result:', { exercises, exercisesError });

      if (exercisesError) throw exercisesError;
      if (!exercises) {
        throw new Error(`Exercise not found: "${exerciseTitle}". Vérifiez que l'exercice existe et est actif.`);
      }
...
async function getOrCreateTodayWorkout(supabase: ReturnType<typeof createClient>) {
  ...
  console.log('[getOrCreateTodayWorkout] Active workout check:', { activeWorkout, activeError });
  ...
  console.log('[getOrCreateTodayWorkout] Today workout check:', { todayWorkout, todayError });
  ...
  console.log('[getOrCreateTodayWorkout] New workout created:', { newWorkout, error });
